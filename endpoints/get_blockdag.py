...
class BlockdagResponse(BaseModel):
    networkName: str = Field(..., example="stokes-mainnet")
...
@app.get("/info/network", response_model=BlockdagResponse, tags=["Stokes network info"], deprecated=True)
...
@app.get("/info/blockdag", response_model=BlockdagResponse, tags=["Stokes network info"])
async def get_blockdag():
    """
    Get Stokes BlockDAG information
    """
    rpc_client = await kaspad_rpc_client()
    if rpc_client:
        info = await wait_for(rpc_client.get_block_dag_info(), 10)
        info["networkName"] = f"stokes-{info['network']}"
        return info
    else:
